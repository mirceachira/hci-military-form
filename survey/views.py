...
def user_info_form_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            experienta_functie= str(durata_functie_ani) + " ani " + str(durata_functie_luni) + " luni "
            experienta_unitate= str(durata_unitate_ani) + " ani " + str(durata_unitate_luni) + " luni "
...
def user_code_form_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cod_utilizator = litera_oras + litera_judet + str(suma_cifre_an_nastere) + str(suma_cifre_zi_luna)
            
            # TO DO : redierect this where it needs to be.
            return render(request, 'cod_utilizator.html', {'form' : form, 'cod' : cod_utilizator})
